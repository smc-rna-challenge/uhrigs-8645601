...
- {class: DockerRequirement, dockerImageId: '', dockerPull: 'quay.io/smc-rna-challenge/uhrigs-8645601-smc-rna-challenge-4:1.0.0'}
id: https://cgc-api.sbgenomics.com/v2/apps/uhrigs/smc-rna-challenge-4/arriba-0-8-sensitive/1/raw/
inputs:
...
- class: CreateFileRequirement
  ...
  - {fileContent: "#!/bin/bash\n\nset -o pipefail\n\nif [ $# -ne 8 ]; then\n\techo\
      ...
      \ 10 \\\n\t--limitBAMsortRAM ${MEMORY}000000 |\n\"$TOOLS_DIR/extract_read-through_fusions\"\
      \ -g \"$REFERENCE_GTF\" -G \"gene_name=gene_name gene_id=gene_id transcript_id=transcript_id\
      ...
      .[0-9]+$/ && $2 ~ /^AC[0-9]+\\.[0-9]+$/) &&\n\t$1 != $2\n' \"$FUSIONS_OUT.tsv\"\
      \ > \"$FUSIONS_OUT.filtered.tsv\" || exit 1\n\n# convert to BEDPE\n\"$TOOLS_DIR/awk\"\
      \ '\nBEGIN{ OFS=\"\\t\" }\n{\n\tchromosome1 = $5; position1 = $5;\n\tsub(/:.*/,\
      \ \"\", chromosome1); sub(/.*:/, \"\", position1);\n\tchromosome2 = $6; position2\
      \ = $6;\n\tsub(/:.*/, \"\", chromosome2); sub(/.*:/, \"\", position2);\n\tif\
      \ ($10 == \"downstream\") position1--;\n\tif ($11 == \"downstream\") position2--;\n\
      \tstrand1 = $3; if (strand1 ~ /\\.$/) { sub(/\\/.*/, \"\", strand1) } else {\
      \ sub(/.*\\//, \"\", strand1) }\n\tstrand2 = $4; if (strand2 ~ /\\.$/) { sub(/\\\
      /.*/, \"\", strand2) } else { sub(/.*\\//, \"\", strand2) }\n\tfilters = $18;\n\
...
sbg:createdOn: 1491758461
sbg:id: uhrigs/smc-rna-challenge-4/arriba-0-8-sensitive/1
sbg:image_url: null
sbg:job:
...
sbg:modifiedOn: 1491758529
sbg:project: uhrigs/smc-rna-challenge-4
sbg:projectName: smc-rna-challenge-4
sbg:revision: 1
sbg:revisionsInfo:
- {'sbg:modifiedBy': uhrigs, 'sbg:modifiedOn': 1491758461, 'sbg:revision': 0, 'sbg:revisionNotes': Copy
    of uhrigs/smc-rna-challenge-4/arriba-0-8/2}
- {'sbg:modifiedBy': uhrigs, 'sbg:modifiedOn': 1491758529, 'sbg:revision': 1, 'sbg:revisionNotes': null}
